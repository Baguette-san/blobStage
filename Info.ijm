...
function launch(){
	...
	File.append("name;unit;pixelW;pixelH;pixelZ;width;height;channels;frames;slices;min;max;moy", outputPathCSV);
...
function ouverture(file_path_curr, file_path_next,fichier){
	...
	ret_csv = newArray(name,unit,pixelW,pixelH,pixelZ,width,height,channels,frames,slices,min,max,moy);
	csvString = "";
