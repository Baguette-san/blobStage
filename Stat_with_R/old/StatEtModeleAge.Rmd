...
ModelnucBy10Mic3 <- 
	lmer(nucBy10Mic3.t~AgeW*Spec+(1|ID2),
			   data = DATA_AGE)
...
```{r }
aggregate(nucBy10Mic3 ~ AgeW + Spec, data = DATA_AGE, FUN = mean)
aggregate(nucBy10Mic3 ~ AgeW + Spec, data = DATA_AGE, FUN = sd)
aggregate(nucBy10Mic3 ~ Spec, data = DATA_AGE, FUN = mean)
aggregate(nucBy10Mic3 ~ Spec, data = DATA_AGE, FUN = sd)
```
...
GraphnucBy10Mic3 <- 
	ggboxplot(DATA_AGE, x = "Spec", y = "nucBy10Mic3", 
			  color = "Spec", palette = "jco", add = "jitter") + 
...
```{r ,echo=CODE, result=CODE,message=FALSE}
ModelmeanAreaNuc <- 
     lmer(meanAreaNuc.t ~ AgeW * Spec + (1 |ID2),
                data = DATA_AGE)
...
```{r }
aggregate(meanAreaNuc ~ AgeW + Spec, data = DATA_AGE, FUN = mean)
aggregate(meanAreaNuc ~ AgeW + Spec, data = DATA_AGE, FUN = sd)
aggregate(meanAreaNuc ~ Spec, data = DATA_AGE, FUN = mean)
aggregate(meanAreaNuc ~ Spec, data = DATA_AGE, FUN = sd)
```
...
```{r, message=FALSE,results=FALSE,warning=FALSE, echo=CODE}
GraphmeanAreaNuc <-
   ggboxplot(DATA_AGE, x = "Spec", y = "meanAreaNuc",
   			 color = "Spec", palette = "jco", add = "jitter") + 
...
```{r }
aggregate(circ ~ AgeW + Spec, data = DATA_AGE, FUN = mean)
aggregate(circ ~ AgeW + Spec, data = DATA_AGE, FUN = sd)
aggregate(circ ~ Spec, data = DATA_AGE, FUN = mean)
aggregate(circ ~ Spec, data = DATA_AGE, FUN = sd)
```
...
```{r ,echo=CODE, result=CODE,message=FALSE}
ModelMitoBy10Mic3 <- lmer(MitoBy10Mic3.t ~ AgeW * Spec +(1 | ID2),
                              data = DATA_AGE)

summary(ModelMitoBy10Mic3)

ModelMitoBy10Mic3_2 <- lmer(MitoBy10Mic3.t ~ AgeW +Spec +(1 | ID2),
                                data = DATA_AGE)
...
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelMitoBy10Mic3)$ID2[, 1], 
       main = "Random effects of ID")
...
```{r }
aggregate(MitoBy10Mic3 ~ AgeW + Spec, data = DATA_AGE, FUN = mean)
aggregate(MitoBy10Mic3 ~ AgeW + Spec, data = DATA_AGE, FUN = sd)
aggregate(MitoBy10Mic3 ~ Spec, data = DATA_AGE, FUN = mean)
aggregate(MitoBy10Mic3 ~ Spec, data = DATA_AGE, FUN = sd)
```
...
```{r ,echo=CODE, result=CODE,message=FALSE}
ModelIntBy10Mic3 <- 
     lmer(IntBy10Mic3.t ~ AgeW * Spec + (1 | ID2), 
                data = DATA_AGE)
...
```{r }
aggregate(IntBy10Mic3 ~ AgeW + Spec, data = DATA_AGE, FUN = mean)
aggregate(IntBy10Mic3 ~ AgeW + Spec, data = DATA_AGE, FUN = sd)
aggregate(IntBy10Mic3 ~ Spec, data = DATA_AGE, FUN = mean)
aggregate(IntBy10Mic3 ~ Spec, data = DATA_AGE, FUN = sd)
```
