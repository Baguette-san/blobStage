...
#open file : Data_hermes_Age.txt Ouvre le fichier 
(.txt en tableau séparé par des tabulation)
...
#plot normalized data distribution and normality
...
ModelnucBy10Mic3 <- lmer(nucBy10Mic3.t~AgeW*Cond+(1|ID3), data = DATA_AGE)
summary(ModelnucBy10Mic3)
ModelnucBy10Mic3_2 <- lmer(nucBy10Mic3.t~AgeW+Cond+(1|ID3), data = DATA_AGE)
...
tab_model(ModelnucBy10Mic3_2,
          ...
          dv.labels = "Number of nuclei by 1000 um3 (Normalized)",
          pred.labels = c("Intercept", "Age", "Lu vs Malu"))
```

```{r , echo=CODE, fig.show=CLEAN}
plot(ModelnucBy10Mic3_2)
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelnucBy10Mic3_2)$ID3[, 1], main = "Random effects of ID")
qqnorm(resid(ModelnucBy10Mic3_2), main = "Residuals")
```
moyenne, écart type
```{r }
# Mean
aggregate(nucBy10Mic3 ~ AgeW + Cond, data = DATA_AGE, FUN = mean)
aggregate(nucBy10Mic3 ~ AgeW + Cond, data = DATA_AGE, FUN = sd)
# SD
aggregate(nucBy10Mic3 ~ Cond, data = DATA_AGE, FUN = mean)
aggregate(nucBy10Mic3 ~ Cond, data = DATA_AGE, FUN = sd)
```
...
GraphnucBy10Mic3 <- 
	ggboxplot(DATA_AGE, x = "Cond", y = "nucBy10Mic3", 
				 color = "Cond", palette = "jco", add = "jitter") + 
	labs(x = "Strain", y = "Number of nuclei by 1000 µm³") +
	scale_x_discrete(breaks = c("Jeune", "Vieux", "MaLu"), labels = c("LuJ", "LuV", "Malu")) +
	stat_summary(geom = "point", shape = 20, size = 10, color = "grey", fill = "grey") +
...
AgeW.labs <- c("1 week", "52 weeks", "104 weeks")

names(AgeW.labs) <- c("1", "52", "104")
...
```{r ,echo=CODE, result=CODE,message=FALSE}
ModelmeanAreaNuc <- 
     lmer(meanAreaNuc.t ~ AgeW * Strain + (1 |ID3),
                data = DATA_AGE)
...
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelmeanAreaNuc)$ID3[, 1],
...
```{r }
aggregate(meanAreaNuc ~ AgeW + Cond, data = DATA_AGE, FUN = mean)
aggregate(meanAreaNuc ~ AgeW + Cond, data = DATA_AGE, FUN = sd)
aggregate(meanAreaNuc ~ Cond, data = DATA_AGE, FUN = mean)
aggregate(meanAreaNuc ~ Cond, data = DATA_AGE, FUN = sd)
```
...
```{r , message=FALSE,results=FALSE,echo=CODE, fig.show=CLEAN}
ggdensity(DATA_AGE$circ, 
          main = "Density plot for circularity", 
          xlab = "Circularity")
ggqqplot(DATA_AGE$circ)
shapiro.test(DATA_AGE$circ)
```

#normalize variable circ

```{r , message=FALSE,results=FALSE,echo=CODE, fig.show=CLEAN}
bestNormalize(DATA_AGE$circ, allow_orderNorm = FALSE)
circBN <- bestNormalize(DATA_AGE$circ)
MASS::truehist(DATA_AGE$circ)
MASS::truehist(circBN$x.t)
DATA_AGE$circ.t <- circBN$x.t
...
#plot normalized data distribution and normality, variable circ

```{r , message=FALSE,results=FALSE,echo=CODE, fig.show=CLEAN}
ggdensity(DATA_AGE$circ.t, 
          main = "Density plot for circularity Normalized", 
          xlab = "Circularity (Normalized)")

ggqqplot(DATA_AGE$circ.t)
shapiro.test(DATA_AGE$circ.t)
```
...
plot(ModelCirc)
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelCirc)$ID3[, 1], 
       main = "Random effects of ID")
...
```{r }
aggregate(circ ~ AgeW + Cond, data = DATA_AGE, FUN = mean)
aggregate(circ ~ AgeW + Cond, data = DATA_AGE, FUN = sd)
aggregate(circ ~ Cond, data = DATA_AGE, FUN = mean)
aggregate(circ ~ Cond, data = DATA_AGE, FUN = sd)
```
...
```{r ,echo=CODE, result=CODE,message=FALSE}
ModelMitoBy10Mic3 <- lmer(MitoBy10Mic3.t ~ AgeW * Strain +(1 | ID3),
                              data = DATA_AGE)

summary(ModelMitoBy10Mic3)

ModelMitoBy10Mic3_2 <- lmer(MitoBy10Mic3.t ~ AgeW +Strain +(1 | ID3),
                                data = DATA_AGE)
...
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelMitoBy10Mic3)$ID3[, 1], 
       main = "Random effects of ID")

qqnorm(resid(ModelMitoBy10Mic3), 
	   main = "Residuals")
```
...
```{r }
aggregate(MitoBy10Mic3 ~ AgeW + Cond, data = DATA_AGE, FUN = mean)
aggregate(MitoBy10Mic3 ~ AgeW + Cond, data = DATA_AGE, FUN = sd)
aggregate(MitoBy10Mic3 ~ Cond, data = DATA_AGE, FUN = mean)
aggregate(MitoBy10Mic3 ~ Cond, data = DATA_AGE, FUN = sd)
```
...
```{r ,echo=CODE, result=CODE,message=FALSE}
ModelIntBy10Mic3 <- 
     lmer(IntBy10Mic3.t ~ AgeW * Strain + (1 | ID3), 
                data = DATA_AGE)
...
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelIntBy10Mic3)$ID3[, 1], 
       main = "Random effects of ID")
...
```{r }
aggregate(IntBy10Mic3 ~ AgeW + Cond, data = DATA_AGE, FUN = mean)
aggregate(IntBy10Mic3 ~ AgeW + Cond, data = DATA_AGE, FUN = sd)
aggregate(IntBy10Mic3 ~ Cond, data = DATA_AGE, FUN = mean)
aggregate(IntBy10Mic3 ~ Cond, data = DATA_AGE, FUN = sd)
```
