...
#open file : Data_hermes_Age.txt Ouvre le fichier (.txt en tableau séparé par des tabulation)
...
#plot data distribution and normality test, variable nucByMic3 1. distribution du nombre de noyau/mic3 2. visualisation obs/théorique 3. test de Shapiro (nb de noyaux / micron)
...
#plot normalized data distribution and normality, variable nucByMic3 vérification normalité sur les données transformées
...
#run LMM , variable nucByMic3 normalized création de 2 modèles mixtes linéaires AnOVa sur les deux modèles (j'ai rajouté le refit = FALSE sinon l'anova faisait un refit en ML)

```{r }
ModelnucByMic3 <- lme4::lmer(nucByMic3.t~AgeW*Spec+(1|ID2),
                             data = DATA_AGE)
summary(ModelnucByMic3)
ModelnucByMic3_2 <- lme4::lmer(nucByMic3.t~AgeW+Spec+(1|ID2),
                               data = DATA_AGE)
summary(ModelnucByMic3_2)
anova(ModelnucByMic3,
      ModelnucByMic3_2, 
      test = 'F', 
      refit = FALSE)
```
...
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelnucByMic3_2)$ID2[, 1], 
...
```{r }
GraphnucByMic3 <- ggboxplot(DATA_AGE,
                            x = "Spec",
                            y = "nucByMic3",
                            color = "Spec",
                            palette = "jco",
...
```{r }
ggdensity(DATA_AGE$MitoByMic3, 
          main = "Density plot for the number of mitochondria by µm³", xlab = "Number of mitochondria by µm³")
...
```{r }
ggdensity(DATA_AGE$MitoByMic3.t, 
          main = "Density plot for the number of mitochondria by µm³",
          xlab = "Number of mitochondria by µm³ (Normalized)")
...
```{r }
ModelMitoByMic3 <- lme4::lmer(MitoByMic3.t ~ AgeW * Spec +(1 | ID2),
                              data = DATA_AGE)
summary(ModelMitoByMic3)
ModelMitoByMic3_2 <- lme4::lmer(MitoByMic3.t ~ AgeW +Spec +(1 | ID2),
                                data = DATA_AGE)
summary(ModelMitoByMic3_2)
anova(ModelMitoByMic3, 
      ModelMitoByMic3_2, 
      test = 'F')
...
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelMitoByMic3)$ID2[, 1], 
       main = "Random effects of ID")
...
GraphMitoByMic3 <- ggboxplot(
     ...
     labs(x = "Strain",
          y = "Number of mitochondria by µm³") +
     scale_x_discrete(breaks = c("Lu", "MaLu"),
                      labels = c("Lu", "Malu")) +
     stat_summary(
          fun.y = mean,
...
names(AgeW.labs) <- c("1", "52")
GraphMitoByMic3T <-
     GraphMitoByMic3 + facet_grid(. ~ AgeW, labeller = labeller(AgeW = AgeW.labs)) + theme(strip.background = element_rect(colour = "black", fill = "white"))
...
```{r }
ModelmeanAreaNuc <- 
     lme4::lmer(meanAreaNuc.t ~ AgeW * Spec + (1 |ID2),
                data = DATA_AGE)
...
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelmeanAreaNuc)$ID2[, 1],
       main = "Random effects of ID")
qqnorm(resid(ModelmeanAreaNuc), 
       main = "Residuals")
```
...
GraphmeanAreaNucT <- GraphmeanAreaNuc + 
     facet_grid(. ~ AgeW, 
                labeller = labeller(AgeW = AgeW.labs)) + 
  theme(strip.background = element_rect(colour = "black", 
                                        fill = "white"))
...
```{r }
ggdensity(DATA_AGE$IntByMic3.t, 
...
```{r }
ModelIntByMic3 <- 
     lme4::lmer(IntByMic3.t ~ AgeW * Spec + (1 | ID2), 
                data = DATA_AGE)
...
plot(ModelIntByMic3)
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelIntByMic3)$ID2[, 1], 
       main = "Random effects of ID")
qqnorm(resid(ModelIntByMic3), 
       main = "Residuals")
```
...
AgeW.labs <- c("1 week", "52 weeks")
names(AgeW.labs) <- c("1", "52")
GraphIntByMic3T <- GraphIntByMic3 + 
  facet_grid(. ~ AgeW, labeller = labeller(AgeW = AgeW.labs)) + 
  theme(strip.background = element_rect(colour = "black", 
                                        fill = "white"))
...
```{r }
ggdensity(DATA_AGE$IntMeanRescale, 
          main = "Density plot for Mitosox signal by µm³", 
          xlab = "Mitosox signal by µm³")
...
```{r }
ggdensity(DATA_AGE$IntMeanRescale.t, 
          main = "Density plot for the Mitosox signal by µm³ Normalized", 
          xlab = "Mitosox signal by µm³ (Normalized)")
...
```{r }
ModelIntMeanRescale <-
   lme4::lmer(IntMeanRescale.t ~ AgeW * Spec + (1 | ID2), 
              data = DATA_AGE)
...
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelIntMeanRescale)$ID2[, 1], 
       main = "Random effects of ID")
qqnorm(resid(ModelIntMeanRescale), 
       main = "Residuals")
```
...
```{r }
GraphIntMeanRescale <- ggboxplot(DATA_AGE,
								 x = "Spec",
								 y = "IntMeanRescale",
								 color = "Spec",
								 palette = "jco",
...
```{r }
ggdensity(DATA_RECOVERY$nucByMic3, 
          main = "Density plot for the number of nuclei by µm³", 
          xlab = "Number of nuclei by µm³")
...
boxplot(log10(nucByMic3BN$oos_preds), 
        yaxt = 'n')
axis(2, 
     at = log10(c(.1, .5, 1, 2, 5, 10)), 
     labels = c(.1, .5, 1, 2, 5, 10))
...
```{r }
ggdensity(DATA_RECOVERY$nucByMic3.t, 
          main = "Density plot for the number of nuclei by µm³ Normalized", 
          xlab = "Number of nuclei by µm³ (Normalized)")
...
```{r }
ModelnucByMic3 <-
   lme4::lmer(nucByMic3.t ~ Awake * Spec + (1 | ID2), 
              data = DATA_RECOVERY)
summary(ModelnucByMic3)
ModelnucByMic3_2 <-
   lme4::lmer(nucByMic3.t ~ Awake + Spec + (1 | ID2), 
...
```{r }
ggdensity(DATA_RECOVERY$MitoByMic3, 
          main = "Density plot for the number of mitochondria by µm³",
          xlab = "Number of mitochondria by µm³")
...
```{r }
ggdensity(DATA_RECOVERY$MitoByMic3.t, 
          main = "Density plot for the number of mitochondria by µm³",
          xlab = "Number of mitochondria by µm³ (Normalized)")
...
```{r }
ModelMitoByMic3 <-
   lme4::lmer(MitoByMic3.t ~ Awake * Spec + (1 | ID2), 
              data = DATA_RECOVERY)
summary(ModelMitoByMic3)
ModelMitoByMic3_2 <-
   lme4::lmer(MitoByMic3.t ~ Awake + Spec + (1 | ID2), 
...
anova(ModelMitoByMic3_2, 
      type = 3, 
      ddf = "Satterthwaite")

tab_model(ModelMitoByMic3_2,
          show.df = TRUE,
          show.stat = TRUE,
          show.loglik = TRUE,
          dv.labels = "Number of mitochondria by µm³ (Normalized)",
...
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelMitoByMic3_2)$ID2[, 1], 
       main = "Random effects of ID")
qqnorm(resid(ModelMitoByMic3_2), 
       main = "Residuals")
```
...
```{r }
GraphMitoByMic3 <- ggboxplot(DATA_RECOVERY,
                             x = "Spec",
                             y = "MitoByMic3",
                             color = "Spec",
                             palette = "jco",
...
```{r }
ModelmeanAreaNuc <-
   lme4::lmer(meanAreaNuc.t ~ Awake * Spec + (1 | ID2), 
              data = DATA_RECOVERY)
summary(ModelmeanAreaNuc)
ModelmeanAreaNuc_2 <-
   lme4::lmer(meanAreaNuc.t ~ Awake + Spec + (1 | ID2), 
...
anova(ModelmeanAreaNuc_2, 
      type = 3, 
      ddf = "Satterthwaite")

tab_model(ModelmeanAreaNuc_2,
          show.df = TRUE,
          show.stat = TRUE,
          show.loglik = TRUE,
          dv.labels = "Area of a Nucleus (Normalized)",
...
```{r }
ggdensity(DATA_RECOVERY$IntByMic3, 
          main = "Density plot for Mitosox signal by µm³", 
          xlab = "Mitosox signal by µm³")
...
```{r }
ggdensity(DATA_RECOVERY$IntByMic3.t, 
          main = "Density plot for the Mitosox signal by µm³ Normalized", 
          xlab = "Mitosox signal by µm³ (Normalized)")
...
```{r }
ModelIntByMic3 <-
   lme4::lmer(IntByMic3.t ~ Awake * Spec + (1 | ID2), 
              data = DATA_RECOVERY)
...
```{r }
GraphIntByMic3 <- ggboxplot(DATA_RECOVERY,
                            x = "Spec",
                            y = "IntByMic3",
                            color = "Spec",
                            palette = "jco",
...
```{r}
ModelIntMeanRescale <-
     lme4::lmer(IntMeanRescale.t ~ Awake * Spec + (1 | ID2), 
                data = DATA_RECOVERY)
...
```{r }
GraphIntMeanRescale <- ggboxplot(DATA_RECOVERY,
                                 x = "Spec",
                                 y = "IntMeanRescale",
                                 color = "Spec",
                                 palette = "jco",
