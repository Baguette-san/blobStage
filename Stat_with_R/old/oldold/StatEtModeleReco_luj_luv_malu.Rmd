...
ModelnucBy10Mic3 <-
   lmer(nucBy10Mic3.t ~ Awake * Cond + (1 | ID3), 
              data = DATA_RECOVERY)
...
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelnucBy10Mic3_2)$ID3[, 1],
...
```{r }
aggregate(nucBy10Mic3 ~ Awake + Cond, data = DATA_RECOVERY, FUN = mean)
aggregate(nucBy10Mic3 ~ Awake + Cond, data = DATA_RECOVERY, FUN = sd)
aggregate(nucBy10Mic3 ~ Cond, data = DATA_RECOVERY, FUN = mean)
aggregate(nucBy10Mic3 ~ Cond, data = DATA_RECOVERY, FUN = sd)
```
...
```{r ,echo=CODE, result=CODE,message=FALSE}
ModelmeanAreaNuc <-
   lmer(meanAreaNuc.t ~ Awake * Cond + (1 | ID3), 
              data = DATA_RECOVERY)
...
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelmeanAreaNuc_2)$ID3[, 1], 
       main = "Random effects of ID")
...
```{r }
aggregate(meanAreaNuc ~ Awake + Cond, data = DATA_RECOVERY, FUN = mean)
aggregate(meanAreaNuc ~ Awake + Cond, data = DATA_RECOVERY, FUN = sd)
aggregate(meanAreaNuc ~ Cond, data = DATA_RECOVERY, FUN = mean)
aggregate(meanAreaNuc ~ Cond, data = DATA_RECOVERY, FUN = sd)
```
...
```{r }
aggregate(circ ~ Awake + Cond, data = DATA_RECOVERY, FUN = mean)
aggregate(circ ~ Awake + Cond, data = DATA_RECOVERY, FUN = sd)
aggregate(circ ~ Cond, data = DATA_RECOVERY, FUN = mean)
aggregate(circ ~ Cond, data = DATA_RECOVERY, FUN = sd)
```
...
```{r ,echo=CODE, result=CODE,message=FALSE}
ModelMitoBy10Mic3 <-
   lmer(MitoBy10Mic3.t ~ Awake * Cond + (1 | ID3), 
              data = DATA_RECOVERY)
...
par(mfrow = c(1, 2))
qqnorm(lme4::ranef(ModelMitoBy10Mic3_2)$ID3[, 1], 
       main = "Random effects of ID")
...
```{r }
aggregate(MitoBy10Mic3 ~ Awake + Cond, data = DATA_RECOVERY, FUN = mean)
aggregate(MitoBy10Mic3 ~ Awake + Cond, data = DATA_RECOVERY, FUN = sd)
aggregate(MitoBy10Mic3 ~ Cond, data = DATA_RECOVERY, FUN = mean)
aggregate(MitoBy10Mic3 ~ Cond, data = DATA_RECOVERY, FUN = sd)
```
...
```{r ,echo=CODE, result=CODE,message=FALSE}
ModelIntBy10Mic3 <-
   lmer(IntBy10Mic3.t ~ Awake * Cond + (1 | ID3), 
              data = DATA_RECOVERY)
...
```{r }
aggregate(IntBy10Mic3 ~ Awake + Cond, data = DATA_RECOVERY, FUN = mean)
aggregate(IntBy10Mic3 ~ Awake + Cond, data = DATA_RECOVERY, FUN = sd)
aggregate(IntBy10Mic3 ~ Cond, data = DATA_RECOVERY, FUN = mean)
aggregate(IntBy10Mic3 ~ Cond, data = DATA_RECOVERY, FUN = sd)
```
