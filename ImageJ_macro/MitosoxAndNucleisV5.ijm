 ...
 */
function launch(){
	resLaunch = launcherStart();
	lifToTiff = resLaunch[0];
	tifToCount = resLaunch[1];
	...
	if(tifToCount){
		...
		//outputPathCSV_nuc = File.openDialog("Select output Blue CSV file ..."); // comment for denying the every nuc output
		File.append("name;unit;pxW;pxH;pxZ;width;height;chan;frames;slices;nucArea;diam;marge;totThick;nucSlices;totPlainNuc;slicesLeft;totNucNotCorr;totMitoNotCorr;corrTotNuc;meanAreaNuc;AR;sumPxInt0;beyondTh16;sumPxInt16;beyondTh32;sumPxInt32", outputPathCSV);
		//File.append("name;proj;Area;X;Y;Circ;AR;Round;Solidity",outputPathCSV_nuc); // comment for denying the every nuc output
		ret_arr = GUI(); // --> GUI(), nucleusArea (px), mitoArea (px), diameter 
	...
	// gestion temps
	temps = timeFrag(getTime());
	print(temps);
	if(tifToCount) File.append(temps,outputPathCSV);
}

/** INPUT START
 * to select which function you want to use :
 * - extract .tif files from archive
 * OR 
 * - run the count and measure functions on the bank
 */
function launcherStart (){
	...
	return newArray( lifToTiff, tifToCount);
}

/** FORMATTAGE DU TEMPS
 * > take a time from an getTime function
 * > return a string with the time in hour / minutes / seconds
 */
function timeFrag(endTime){
	totalTime = endTime - startTime;
	totalSeconds = totalTime / 1000;
	hours = totalSeconds / 3600;
	minutes = (totalSeconds % 3600) / 60;
	seconds = totalSeconds % 60;
	return("Fin du programme en : " + round(hours) + " h " + round(minutes) + " min " + round(seconds) + " s ");
}
 ...
 * note : fonctionne avec la plupart des fichiers archive mais il faut remplacer dans la condition le '.lif' par la nouvelle dénomination
 */
function forAllLif(lif_dir, tif_dir) {
	LIF_file_list = getFileList(lif_dir);
	for (i=0; i<LIF_file_list.length; i++) { 
		if (endsWith(LIF_file_list[i], ".lif")){ // ici pour changer le type d'archive
...
function gestionFichiers(lif_dir, LIF_file_name, outPath) {
	...
	run("Bio-Formats Macro Extensions"); // nécessite le plug-in Bioformat
	Ext.setId(filePath);
 ...
 * (propre au projet) diamètre minimum du noyau par défaut à trois pour être sensiblement en deça des tailles observées
 */
function GUI() { 
	Dialog.create("Parameters ajusting");
	Dialog.addNumber("Nucleis's Area above (pixels) will be counted :", 1000);
...
function splitProjections(name){
	...
	totalNotCorr = 0; totalNucNotCorr = 0; totalMitoNotCorr = 0; meanArea = 0; AR = 0;
	...
	csvArray = newArray(name,unit,pixelW,pixelH,pixelZ,width,height,channels,frames,slices,nucleus_area,diameter,marge,totalThick,nucleusSlices,totalPlainNucleus,slicesLeft,totalNucNotCorr,totalMitoNotCorr,corrTotaleNuc,meanArea,AR,sumPxInt0,beyondTh16,sumPxInt16,beyondTh32,sumPxInt32);
	File.append(CSVPrep(csvArray), outputPathCSV);	
...
function measurementBlue(k) { 
	...
	numbers = CountNucleisAreaLessThan("labels",k);			// --> CountNucleisAreaLessThan
	...
	return newArray(arrEnd,total_nuc,total_mito,meanArea,AR); 			// --> splitProjections
}

/** COMPTAGE par TAILLE
 * 
 */
function CountNucleisAreaLessThan(image, proj){
...
//return the number of object counted
	...
	for (i = 0; i < nResults; i++) {
	    obj_area=getResult("Area",i);	  
	    if(obj_area>=nucleus_area){ 
	    	nuc_number++; 
	    	tot_area+=obj_area; 
	    	/*nucProp(name,proj,i);*/ 
	    	AR+=getResult("AR", i);
	    }
	    else { if(obj_area>=50 && obj_area<=mito_area)	mito_number++; }
	    //Table.deleteRows(nResults-1,nResults-1);
		//updateResults();
	}
	meanArea=tot_area/nuc_number;
	AR = AR / nuc_number;
	return newArray(nuc_number,mito_number,meanArea, AR);
}



function nucProp(name,proj,row) { 
	out = newArray(name,proj+1,getResult("Area", row),getResult("X", row),getResult("Y", row),getResult("Circ.", row),getResult("AR", row),getResult("Round", row),getResult("Solidity", row));
	File.append(CSVPrep(out), outputPathCSV_nuc);
}
...
function CSVPrep(csvArray){
	csvString = "";
	for (i = 0; i < csvArray.length-1; i++) csvString = csvString + csvArray[i] + ";" ;
	out = csvString + csvArray[csvArray.length-1];
	return out;
}