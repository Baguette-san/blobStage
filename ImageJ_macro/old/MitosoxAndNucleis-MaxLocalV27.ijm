...
function launch(){
	...
	if(tifToCount){
		...
		File.append("name;unit;pixelW;pixelH;pixelZ;width;height;channels;frames;slices;nucleus_area;diameter;marge;totalThick;nucleusSlices;totalPlainNucleus;slicesLeft;totalNucNotCorr;totalMitoNotCorr;corrTotaleNuc;sumPxInt0;beyondTh16;sumPxInt16;beyondTh32;sumPxInt32", outputPathCSV);
...
function splitProjections(name){
	...
	csvArray = newArray(name,unit,pixelW,pixelH,pixelZ,width,height,channels,frames,slices,nucleus_area,diameter,marge,totalThick,nucleusSlices,totalPlainNucleus,slicesLeft,totalNucNotCorr,totalMitoNotCorr,corrTotaleNuc,sumPxInt0,beyondTh16,sumPxInt16,beyondTh32,sumPxInt32);
	csvString = "";
...
function measurementRed(k) { 
	...
	for (x = 0; x < width; x++) {
		for (y = 0; y < height; y++) {
			...
			if (intensity>16){
				beyondTh16++;
				sumPxInt16+=intensity;
			}
			if (intensity>32){
				beyondTh32++;
				sumPxInt32+=intensity;
			}
		}
	}
