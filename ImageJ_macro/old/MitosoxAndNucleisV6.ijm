...
function launch(){
	...
	if(tifToCount){
		...
		File.append("name;unit;pxW;pxH;pxZ;width;height;chan;frames;slices;nucArea;diam;marge;totThick;nucSlices;totPlainNuc;slicesLeft;totNucNotCorr;totMitoNotCorr;corrTotNuc;meanAreaNuc;circ;sumPxInt0;beyondTh10000;sumPxInt10000;beyondTh30000;sumPxInt30000", outputPathCSV);
		//File.append("name;proj;Area;X;Y;Circ;AR;Round;Solidity",outputPathCSV_blue);
		ret_arr = GUI();
...
function splitProjections(name){
	...
	totalNotCorr = 0; totalNucNotCorr = 0; totalMitoNotCorr = 0; meanArea = 0; circ = 0;
	...
	for (i = 1; i <= (nSlices-slicesLeft); i++) {
		...
		beyondTh10000 += redMeasure[1];
		sumPxInt10000 += redMeasure[2];
		beyondTh30000 += redMeasure[3];
		sumPxInt30000 += redMeasure[4];
	}
	runClose("red",0);
	setBatchMode(false);
	
	csvArray = newArray(name,unit,pixelW,pixelH,pixelZ,width,height,channels,frames,slices,nucleus_area,diameter,marge,totalThick,nucleusSlices,totalPlainNucleus,slicesLeft,totalNucNotCorr,totalMitoNotCorr,corrTotaleNuc,meanArea,circ,sumPxInt0,beyondTh10000,sumPxInt10000,beyondTh30000,sumPxInt30000);
...
function measurementBlue(k) { 
	...
	return newArray(arrEnd,total_nuc,total_mito,meanArea,circ); 			// --> splitProjections
}
...
//return the number of object counted
	...
	circ = circ / nuc_number;
	return newArray(nuc_number,mito_number,meanArea, circ);
}
...
function nucProp(name,proj,row) { 
	...
	File.append(CSVPrep(out), outputPathCSV_blue);
}
...
function measurementRed(k) {
	...
	for (i = thres1; i < thres2; i++) {
		...
		beyondTh10000+=counts[i];
		sumPxInt10000+=counts[i]*i;
	}
	for (i = thres2; i < 65536; i++) {
		sumPxInt0+=counts[i]*i;
		beyondTh10000+=counts[i];
		sumPxInt10000+=counts[i]*i;
		beyondTh30000+=counts[i];
		sumPxInt30000+=counts[i]*i;
	}
	return newArray(sumPxInt0,beyondTh10000,sumPxInt10000,beyondTh30000,sumPxInt30000); // --> splitProjections
}
